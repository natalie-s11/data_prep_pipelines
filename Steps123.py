...
# %%
# Step 1: Review these two datasets and brainstorm problems that could be
...
College['basic'] = College['basic'].apply(
    lambda x: True if x in basic_categories else pd.NA)
...
Job['workex'] = Job['workex'].apply(
    lambda x: True if x == 'Yes' else (
        False if x == 'No' else pd.NA))
...
# I decided to not do this for College. I was considering it for basic but
# I do not know if certain research is 2 or 4 years so I chose to not
# collapse factor levels.

# Job
# I decided to not do this for Job because the dataset is smaller and
# mostly complete.
...
Job.drop(['sl_no',
         ...
         inplace=True,
         errors='ignore')
# Drop the rows in salary where it is Nan. This is because they are not
# placed so they do not have a salary since they do not have a job.
...
# The target variable is the amount of people per state that go to private
# school.
College['private_school'] = College['control_Private for-profit'] + \
    College['control_Private not-for-profit']
# Calculate private school students per college
College['private_students'] = College['private_school'] * \
    College['cohort_size']
# Group by state and sum to get total private students per state
private_by_state = College.groupby(
    'state')['private_students'].sum().sort_values(ascending=False)
...
Job['salary_group'] = pd.qcut(
    Job['salary'], q=3, labels=[
        'Low', 'Medium', 'High'])
# Then, see how concentrations and majors distribute across the salary
# groups I just created
major_counts = (
    Job
...
# Job
# Calculate which major and concentration are the most common in the high
# salary group
...
print(
    "Train distribution:\n",
    train['salary_group'].value_counts(
        normalize=True))
print(
    "\nTune distribution:\n",
    tune['salary_group'].value_counts(
        normalize=True))
print(
    "\nTest distribution:\n",
    test['salary_group'].value_counts(
        normalize=True))
